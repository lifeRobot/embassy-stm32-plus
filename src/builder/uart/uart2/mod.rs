...
impl Uart2Builder {
    ...
    #[inline]
    pub fn build_disable(self) -> Result<Uart<'static, USART2, NoDma, NoDma>, ConfigError> {
        self.build_tx(NoDma, NoDma)
    }

    /// build a serial port that only supports read data
    #[inline]
    pub fn build_read(self, read_dma: DMA1_CH6) -> Result<Uart<'static, USART2, NoDma, DMA1_CH6>, ConfigError> {
        self.build_tx(NoDma, read_dma)
    }

    /// build a serial port that only supports write data
    #[inline]
    pub fn build_write(self, write_dma: DMA1_CH7) -> Result<Uart<'static, USART2, DMA1_CH7, NoDma>, ConfigError> {
        self.build_tx(write_dma, NoDma)
    }

    /// build a serial port that supports read and write data
    #[inline]
    pub fn build_all(self, write_dma: DMA1_CH7, read_dma: DMA1_CH6)
                     -> Result<Uart<'static, USART2, DMA1_CH7, DMA1_CH6>, ConfigError> {
    ...
    fn build_tx<TxDma, RxDma>(self, tx_dma: impl Peripheral<P=TxDma> + 'static, rx_dma: impl Peripheral<P=RxDma> + 'static)
                              -> Result<Uart<'static, USART2, TxDma, RxDma>, ConfigError> {
