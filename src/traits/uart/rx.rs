...
/// all uart rx trait
pub trait UartRxAllTrait<Rx, Rts, RxDma = NoDma>: Sized {
    fn build_rx_with_dma_rtscts_config(self, rx: Rx, rts: Rts, rx_dma: RxDma, config: Config) -> UartRx<'static, Async>;

    fn build_rx_with_dma_rtscts(self, rx: Rx, rts: Rts, rx_dma: RxDma) -> UartRx<'static, Async> {
...
pub trait UartRxRtsCtsTrait<Rx, Rts>: UartRxAllTrait<Rx, Rts> {
    fn build_rx_with_rtscts_config(self, rx: Rx, rts: Rts, config: Config) -> UartRx<'static, Async> {
        self.build_rx_with_dma_rtscts_config(rx, rts, NoDma, config)
    }

    fn build_rx_with_rtscts(self, rx: Rx, rts: Rts) -> UartRx<'static, Async> {
        self.build_rx_with_rtscts_config(rx, rts, Config::default())
    }
}

/// dma any uart rx trait
pub trait UartRxDmaAnyTrait<Rx>: Sized {
    fn build_rx_with_dma_config_any<RxDma>(self, rx: Rx, rx_dma: impl Peripheral<P=RxDma> + 'static, config: Config) -> UartRx<'static, Async>;

    fn build_rx_with_dma_any<RxDma>(self, rx: Rx, rx_dma: impl Peripheral<P=RxDma> + 'static) -> UartRx<'static, Async> {
        self.build_rx_with_dma_config_any(rx, rx_dma, Config::default())
    }
}
/// dma uart rx trait
pub trait UartRxDmaTrait<Rx, RxDma = NoDma>: UartRxDmaAnyTrait<Rx> {
    fn build_rx_with_dma_config(self, rx: Rx, rx_dma: impl Peripheral<P=RxDma> + 'static, config: Config) -> UartRx<'static, Async> {
        self.build_rx_with_dma_config_any(rx, rx_dma, config)
    }

    fn build_rx_with_dma(self, rx: Rx, rx_dma: impl Peripheral<P=RxDma> + 'static) -> UartRx<'static, Async> {
...
pub trait UartRxTrait<Rx>: UartRxDmaTrait<Rx> {
    fn build_rx_with_config(self, rx: Rx, config: Config) -> UartRx<'static, Async> {
        self.build_rx_with_dma_config(rx, NoDma, config)
    }

    fn build_rx(self, rx: Rx) -> UartRx<'static, Async> {
