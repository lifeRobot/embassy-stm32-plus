...
/// all uart tx trait
pub trait UartTxAllTrait<Tx, Cts, TxDma = NoDma>: Sized {
    fn build_tx_with_dma_rtscts_config(self, tx: Tx, cts: Cts, tx_dma: TxDma, config: Config) -> UartTx<'static, Async>;

    fn build_tx_with_dma_rtscts(self, tx: Tx, cts: Cts, tx_dma: TxDma) -> UartTx<'static, Async> {
...
pub trait UartTxRtsCtsTrait<Tx, Cts>: UartTxAllTrait<Tx, Cts> {
    fn build_tx_with_rtscts_config(self, tx: Tx, cts: Cts, config: Config) -> UartTx<'static, Async> {
        self.build_tx_with_dma_rtscts_config(tx, cts, NoDma, config)
    }

    fn build_tx_with_rtscts(self, tx: Tx, cts: Cts) -> UartTx<'static, Async> {
        self.build_tx_with_rtscts_config(tx, cts, Config::default())
    }
}

/// dma any uart tx trait
pub trait UartTxDmaAnyTrait<Tx>: Sized {
    fn build_tx_with_dma_config_any<TxDma>(self, tx: Tx, tx_dma: impl Peripheral<P=TxDma> + 'static, config: Config) -> UartTx<'static, Async>;

    fn build_tx_with_dma_any<TxDma>(self, tx: Tx, tx_dma: impl Peripheral<P=TxDma> + 'static) -> UartTx<'static, Async> {
...
pub trait UartTxDmaTrait<Tx, TxDma = NoDma>: UartTxDmaAnyTrait<Tx> {
    fn build_tx_with_dma_config(self, tx: Tx, tx_dma: impl Peripheral<P=TxDma> + 'static, config: Config) -> UartTx<'static, Async> {
        self.build_tx_with_dma_config_any(tx, tx_dma, config)
    }

    fn build_tx_with_dma(self, tx: Tx, tx_dma: impl Peripheral<P=TxDma> + 'static) -> UartTx<'static, Async> {
        self.build_tx_with_dma_config(tx, tx_dma, Config::default())
    }
}
/// simple uart tx trait
pub trait UartTxTrait<Tx>: UartTxDmaTrait<Tx> {
    fn build_tx_with_config(self, tx: Tx, config: Config) -> UartTx<'static, Async> {
        self.build_tx_with_dma_config(tx, NoDma, config)
    }

    fn build_tx_with(self, tx: Tx) -> UartTx<'static, Async> {
